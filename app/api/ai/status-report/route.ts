...
import { generateStatusReportResult } from "@/lib/aiToolsService";
import {
  EntityNotFoundError,
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await generateStatusReportResult({
      entityType,
      entityId,
    ...
    console.error("/api/ai/status-report error:", error);
    return Response.json(
      {
        success: false,
        result: null,
        message: "Failed to generate status report. Please try again.",
      },
      { status: 500 }
    );
  }
